...
### Обязательные связки классов работ (D16)

`work_class` задачи — ось «ЗАЧЕМ», ортогональная `task_type` («КАК»). Два
класса из трёх обязаны нести связку, и она создаётся **вместе с записью**, а не
вторым вызовом:

| Класс | Обязан нести | Как ставится | Список нарушений |
|---|---|---|---|
| `uc` | `REALIZES` → сценарий | `task_new(uc_id=…)` / `task_set(uc_id=…)` | слайс `unlinked_uc_tasks` |
| `enb` | `JUSTIFIED_BY` → ADR | `task_link(rel="adr")` | слайс `unlinked_enb_tasks` |
| `jtd` | — | — | — |

`uc_id` при создании задачи создаёт ребро **атомарно с ней**: несуществующий
сценарий — отказ 400, а не «задача есть, ребра нет». Раньше связка была вторым
вызовом со стороны сценария, и между вызовами существовало окно, в котором
задача жила непривязанной.

Проверки — **advisory**: слайсы показывают нарушения, но запись не блокируют
(гейт задушил бы ввод — TOC-раздел ADR-022). Блокируются только структурные
инварианты: несуществующая цель ребра и цикл в иерархии.

Слайс `tasks_of_uc` отдаёт задачу вместе со **статусом её спринта**
(`sprint_status_raw`), чтобы «доехало ли» читалось в сценарии, без перехода.
