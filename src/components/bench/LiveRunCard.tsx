...
import { useBenchStatus } from '../../hooks/useBench';
import { humanizeSeconds, parseBenchTimestamp } from '../../utils/benchData';
...
export function LiveRunCard() {
  ...
  const { status, stale, unavailable } = useBenchStatus(intervalMs);
  useEffect(() => {
  ...
  const updatedMs = parseBenchTimestamp(status.updated);
