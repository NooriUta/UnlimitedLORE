...
import { normalizePainGainJobId } from './PainGainJobModal';
...
describe('PL-18B · id новой боли/выгоды', () => {
  it('дописывает недостающий префикс', () => {
    ...
    expect(normalizePainGainJobId('pain', 'lore-manual-handoff')).toBe('PAIN-LORE-MANUAL-HANDOFF');
    expect(normalizePainGainJobId('gain', 'lore-linked-releases')).toBe('GAIN-LORE-LINKED-RELEASES');
  });

  it('не удваивает уже поставленный префикс', () => {
    expect(normalizePainGainJobId('pain', 'PAIN-X')).toBe('PAIN-X');
    expect(normalizePainGainJobId('pain', 'pain-x')).toBe('PAIN-X');
  });

  it('пробелы становятся дефисами, края обрезаются', () => {
    expect(normalizePainGainJobId('gain', '  меньше рутины ')).toBe('GAIN-МЕНЬШЕ-РУТИНЫ');
  });
  ...
  it('пустой ввод остаётся пустым, а не превращается в голый префикс', () => {
    ...
    expect(normalizePainGainJobId('pain', '')).toBe('');
    expect(normalizePainGainJobId('pain', '   ')).toBe('');
  });
});
