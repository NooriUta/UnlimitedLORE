...
const TOOLS: ToolDoc[] = [
  ...
  { name: 'lore_update_sprint', kind: 'write', backend: 'POST /lore/sprint/update',
    ...
    desc: 'KnowSprint: частичное обновление метаданных — пишутся только переданные поля, остальные не трогаются. Статус этим тулом не меняется (для статуса — lore_set_status). Правило: всегда заполнять context_md, если известно, зачем спринт существует.' },
  ...
  // ── Status (SCD2-переходы) ──────────────────────────────────────────────
  { name: 'lore_set_status', kind: 'write', backend: 'POST /lore/status',
    params: 'entity_type, id, status',
    desc: 'Сменить статус одной сущности через полный SCD2-переход: закрыть текущую открытую hist-строку (valid_to=now), открыть новую. entity_type = plan_item | sprint | task | checkpoint | phase. status ∈ todo|planned|active|partial|done|blocked|high|cancelled|backlog|design|ready_for_deploy.' },
  ...
  { name: 'lore_create_phase', kind: 'write', backend: 'POST /lore/phase',
    ...
    desc: 'KnowPhase: создать фазу спринта. phase_uid = "<sprint_id>/PHASE_<KEY>", ребро PART_OF → спринт, стартовый статус PLANNED в KnowPhaseHist. Идемпотентно — повторный вызов с тем же phase_key возвращает существующую фазу (created=false), ничего не меняя.' },
  ...
  // ── ADR (KnowADR) / Decision (KnowDecision) ──────────────────────────────
  ...
  { name: 'lore_link_adr_sprint', kind: 'write', backend: 'POST /lore/adr/link',
  ...
  { name: 'lore_link_adr_release', kind: 'write', backend: 'POST /lore/adr/link',
  ...
  { name: 'lore_rename_adr', kind: 'write', backend: 'POST /lore/adr/rename',
  ...
  { name: 'lore_delete_adr', kind: 'write', backend: 'POST /lore/adr/delete',
    params: 'adr_id',
    desc: 'KnowADR: БЕЗВОЗВРАТНО удалить — каскад рёбра → KnowADRHist-строки → вершина. Для любого ADR, который когда-либо был реальным решением, вместо удаления предпочесть status=DEPRECATED/SUPERSEDED через lore_update_adr; delete — только для тестовых артефактов и ошибочных созданий.' },
  { name: 'lore_create_decision', kind: 'write', backend: 'POST /lore/decision',
    params: 'decision_id, title, body_md?, date_created?, refs_raw?',
    desc: 'KnowDecision: создать или обновить (upsert по decision_id). Для фиксации разовых ключевых решений, принятых в ходе спринта или дизайн-сессии — не заменяет ADR, а дополняет их для менее формальных вердиктов.' },

  // ── Spec (KnowSpec) / QualityGate / Runbook (KnowRunbook) / Doc (KnowDoc) ─
  ...
  { name: 'lore_delete_spec', kind: 'write', backend: 'POST /lore/spec/delete',
    params: 'spec_id',
    desc: 'KnowSpec: безвозвратно удалить вершину по spec_id.' },
  { name: 'lore_create_quality_gate', kind: 'write', backend: 'POST /lore/quality-gate',
    ...
    desc: 'QualityGate: создать или обновить (upsert по qg_id) — сам гейт как вершину, с текстом инвариантов в content_md. Прогоны и метрики к этой вершине пишет отдельно lore_record_qg_run.' },
  { name: 'lore_create_runbook', kind: 'write', backend: 'POST /lore/runbook',
    params: 'runbook_id, name, area?, date_created?, content_md?',
    desc: 'KnowRunbook: создать или обновить (upsert по runbook_id) — операционный плейбук/инструкция (area: recovery, infra, deploy, ops, auth, db, service…).' },
  { name: 'lore_create_doc', kind: 'write', backend: 'POST /lore/doc',
    ...
    desc: 'KnowDoc: создать или обновить (upsert по doc_id) — произвольный HTML-документ или фрагмент (страница, гайд, справка…). content_html ограничен 100 КБ.' },
