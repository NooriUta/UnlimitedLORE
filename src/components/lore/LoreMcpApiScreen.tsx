...
const TOOLS: ToolDoc[] = [
  ...
  { name: 'lore_link_adr_sprint', kind: 'write', backend: 'POST /lore/adr/link',
    ...
    desc: 'IMPLEMENTED_IN edge (ADR → спринт-реализатор) — питает implemented_in_ids в слайсе adr. Идемпотентен; action = add | remove. Ответ несёт linked:true/false — false значит adr_id/sprint_id не найдены (ребро не создано).' },
  { name: 'lore_link_adr_release', kind: 'write', backend: 'POST /lore/adr/link',
    params: 'adr_id, release_id, git_project?, action?',
    desc: 'IMPLEMENTED_IN_RELEASE edge (ADR → релиз) — питает release_ids в слайсе adr. С git_project матчит по release_uid (multi-repo safe). linked:false + hint в ответе, если релиз не найден.' },
  { name: 'lore_rename_adr', kind: 'write', backend: 'POST /lore/adr/rename',
    params: 'adr_id, new_adr_id',
    desc: 'Переименовать ADR по месту: рёбра висят на вершине (@rid), поэтому DEPENDS_ON/SUPERSEDES/BELONGS_TO/TAGGED_WITH/IMPLEMENTED_IN*/HAS_STATE выживают без перевешивания. Отказ, если new_adr_id занят. Строковые упоминания старого id в документах НЕ обновляются.' },
  { name: 'lore_delete_adr', kind: 'write', backend: 'POST /lore/adr/delete',
    params: 'adr_id',
    desc: 'БЕЗВОЗВРАТНО удалить ADR: каскад — рёбра → KnowADRHist-строки → вершина. Для когда-либо реальных ADR предпочесть status=DEPRECATED/SUPERSEDED; delete — для тест-артефактов и ошибочных созданий.' },
