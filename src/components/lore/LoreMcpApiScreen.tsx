...
const TOOLS: ToolDoc[] = [
  ...
  { name: 'lore_create_publication', kind: 'write', entity: 'BRAGI', backend: 'POST /lore/bragi/publication',
    params: 'publication_id, title?, topic?, main_text_md?, type?, status_general?, keyword_ids?[]',
    desc: 'BragiPublication: создать/дополнить публикацию (upsert по publication_id, только переданные поля). Мастер-версия, группирующая вариации по площадкам (lore_create_variant). keyword_ids — рёбра TARGETS_KEY на существующие BragiKeyword (аддитивно, не отвязывает пропущенные при повторном вызове).' },
  ...
  { name: 'lore_attach_asset', kind: 'write', entity: 'BRAGI', backend: 'POST /lore/bragi/asset',
    ...
    desc: 'BragiAsset: создать/дополнить изображение/медиа (upsert по asset_id) и опционально привязать через HAS_ASSET к существующей BragiPublication (обложка) или BragiVariant (картинка вариации) — передавать ровно одно из attach_to_*.' },
  { name: 'lore_upsert_keyword', kind: 'write', entity: 'BRAGI', backend: 'POST /lore/bragi/keyword',
    params: 'keyword_id, phrase?, cluster?, freq_exact?, freq_broad?, source?, intent?, region_engine?, measured_at?, page_id?',
    desc: 'BragiKeyword: создать/дополнить ключевую фразу (upsert по keyword_id). page_id → ребро TARGETS_PAGE на существующую BragiPage.' },
