 ...
 * RAGVSDL (see BenchMartResource) — files remain ONLY for:
 *
...
public class BenchResource {
    ...
    public record BenchError(String error, String detail) {}
    ...
    public Response file(@PathParam("path") String path) {
        if (!isAllowedRelPath(path)) {
            return noStore(Response.status(Response.Status.FORBIDDEN)
                ...
                .entity(new BenchError("FORBIDDEN", "path outside whitelist")));
        }
    ...
    private Response rawFile(String relPath, String contentType) {
        ...
        try {
            ...
            return noStore(Response.status(Response.Status.FORBIDDEN)
                .type(MediaType.APPLICATION_JSON)
                .entity(new BenchError("FORBIDDEN", "invalid path")));
        }
        if (!file.startsWith(root)) {
            return noStore(Response.status(Response.Status.FORBIDDEN)
                .type(MediaType.APPLICATION_JSON)
                .entity(new BenchError("FORBIDDEN", "path traversal blocked")));
        }
        if (!Files.isRegularFile(file)) {
            return noStore(Response.status(Response.Status.NOT_FOUND)
                .type(MediaType.APPLICATION_JSON)
                .entity(new BenchError("NOT_FOUND", relPath)));
        }
        try {
    ...
    private Response rootMissing() {
        return noStore(Response.status(Response.Status.NOT_FOUND)
            .type(MediaType.APPLICATION_JSON)
            .entity(new BenchError("BENCH_ROOT_MISSING", benchRoot)));
    }
