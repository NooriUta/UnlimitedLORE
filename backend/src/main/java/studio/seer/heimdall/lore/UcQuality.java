 ...
 *
 * <p><b>Все проверки весят по единице, и это НЕ осознанное решение</b>, а
 * состояние, до которого не дошли руки (MT-01). Отсутствие секции «Триггер» и
 * отсутствие primary-актора дают одинаковый минус, хотя первое — недописанный
 * текст, а второе — сценарий, которому некому исполнять. Если будете вводить
 * веса, начинать стоит отсюда, а не с добавления новых проверок.
 *
 * <p><b>Момент замера имеет значение.</b> {@code primary_actor} входит в
 * знаменатель, но актор — ребро, и привязывается ОТДЕЛЬНЫМ вызовом после
 * {@code uc_new}. Поэтому любой только что созданный сценарий получает 9/10.
 * Чтобы оценка не врала вниз систематически, {@code uc_link rel=actor}
 * пересчитывает вердикт и возвращает его в ответе — см. MT-01.
 */
