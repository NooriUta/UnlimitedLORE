...
class LoreUcQualityBodyTest {
    ...
    /**
     * ПУСТОЙ шаблон не считается заполненным (PL-40).
     *
     * <p>Проверки были структурными — искали заголовок секции. Свежевставленный
     * скелет Кокберна (одни заголовки, курсивные подсказки и «1. …») набирал 7
     * из 10 и показывал «✓» напротив секций, где не написано ни слова. Это хуже
     * нулевой оценки: линтер подтверждал качество текста, которого нет.
     */
    @Test
    void freshTemplateScoresAlmostNothing() {
        String tpl = "### Триггер\\n_Что запускает сценарий._\\n\\n### Предусловия\\n_Что должно быть истинно до старта._\\n\\n"
            + "### Основной сценарий\\n1. …\\n2. …\\n\\n### Расширения\\n_Ветвления вида «2a. …»._\\n\\n"
            + "### Минимальные гарантии\\n_Что истинно даже при неудаче._\\n\\n### Гарантии успеха\\n_Измеримый результат._\\n";
        var r = lint("{\"scenario_md\":\"" + tpl + "\",\"rigor\":\"fully-dressed\",\"goal_level\":\"sea-level\"}");
        int score = r.jsonPath().getInt("score");
        // Единственное, что действительно задано в этом теле — уровень цели.
        org.junit.jupiter.api.Assertions.assertTrue(score <= 1,
            "пустой шаблон не должен набирать баллы за секции с прочерками, набрал " + score);
    }

    /** Заполненные секции засчитываются — иначе лечение оказалось бы хуже болезни. */
    @Test
    void filledSectionsStillCount() {
        String md = "### Триггер\\nАгент открыл PR.\\n\\n### Основной сценарий\\n1. Проверяем CI\\n2. Мержим\\n\\n"
            + "### Минимальные гарантии\\nВетка не сломана.\\n";
        var r = lint("{\"scenario_md\":\"" + md + "\",\"acceptance_md\":\"1. CI зелёный\",\"rigor\":\"casual\",\"goal_level\":\"sea-level\"}");
        org.junit.jupiter.api.Assertions.assertTrue(r.jsonPath().getInt("score") >= 4,
            "содержательно заполненный casual обязан набирать почти всё");
    }
