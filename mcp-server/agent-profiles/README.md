...
7 [OpenCode agent](https://opencode.ai/docs/agents) permission profiles for the `aida-lore` MCP
server's tools. `full` is the `primary` mode profile (Claude / backfill sessions) with unrestricted
`"*": "allow"`. The other 6 are `subagent` mode, each a `"*": "deny"` catch-all plus an allow-list of
...
| `architect` | `adr_*`, `component_*`, `tech_*`, `spec_*`, `runbook_*`, `doc_*`, `decision_*`, `project_new`, `status_set` |
...
| `pm` | `sprint_*`, `task_*`, `milestone_*`, `project_new`, `status_set`, `status_set_batch` |
